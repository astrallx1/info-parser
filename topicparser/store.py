...
class Topic(Base):
    ...
    # 'gh' / 'tw' / 'feed' / 'other', decided ONCE in the ranker from the signal that
    # produced the topic. NULL on rows written before the column existed, and the feed
    # and the export both fall back to sniffing links[0] for those.
...
def _topic_window(links_json: str, source: str | None = None) -> str:
    ...
    if source:
        return source if source in ("tw", "feed") else "gh"
    try:
