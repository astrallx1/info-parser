...
feed URLs. The picker in the feed then lets you include or exclude any single source
before a run, so a quick GitHub-only pass costs a few cents and a minute.
...
same of a blog post: is this the event, or the publisher talking about itself? Both gates
are optional: delete the file and the call never happens.
